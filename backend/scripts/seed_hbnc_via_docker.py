"""
Seed script for HBNC template via Docker
This script generates SQL to insert the HBNC template directly into the database
"""

import json

def generate_hbnc_template_sql():
    """Generate SQL INSERT statement for HBNC template"""
    ...
    questions_json = json.dumps(hbnc_questions).replace("'", "''")
    meta_json = json.dumps(meta_data).replace("'", "''")
    
    sql = f"""
INSERT INTO visit_templates (template_type, name, questions, meta_data, created_at)
VALUES (
    'hbnc',
    'HBNC Standard Template',
...
if __name__ == "__main__":
    print(generate_hbnc_template_sql())
